-- ============================================================================
-- Utility Hub v2.0 — SINGLE FILE BUILD
-- รวมทุกโมดูลในไฟล์เดียว ไม่ต้องมีโฟลเดอร์ modules/
-- วิธีใช้: Execute ไฟล์นี้ตรงๆ ใน executor
-- ============================================================================

local bootLog = {}
local function logBoot(msg)
    bootLog[#bootLog + 1] = "[" .. tostring(tick()) .. "] " .. tostring(msg)
    pcall(function()
        if writefile then
...
logBoot("start (single-file build)")
pcall(function()
    logBoot("has loadstring=" .. tostring(loadstring ~= nil) .. " has load=" .. tostring(load ~= nil))
    logBoot("has gethui=" .. tostring(gethui ~= nil) .. " has writefile=" .. tostring(writefile ~= nil))
end)
...
pcall(function()
    logBoot("TouchEnabled=" .. tostring(UserInputService.TouchEnabled))
    logBoot("Platform viewport ready")
end)
...
local function getSafeParent()
    -- Delta (iOS/Android): ลอง gethui ก่อน แล้ว PlayerGui (เสถียรบน iPad กว่า CoreGui)
    if gethui then
    ...
    return game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui") or CoreGui
end
...
local function ProtectMyUI(gui)
    pcall(function()
        ...
        -- อย่าบังคับย้าย Parent บนมือถือถ้า parent ปัจจุบันใช้ได้แล้ว (ลดบัค iPad)
        if gethui and gui.Parent == nil then
            local ok, h = pcall(gethui)
            if ok and h then
                gui.Parent = h
            end
...
local function createBootMarker()
    pcall(function()
        ...
        lbl.Size = UDim2.new(0, 360, 0, 44)
        lbl.Position = UDim2.new(0.5, -180, 0, 60)
...
local Colors = {
    -- Frosted Glass Gray Theme
    Bg = Color3.fromRGB(40, 40, 44),
    Panel = Color3.fromRGB(55, 55, 60),
    Sidebar = Color3.fromRGB(48, 48, 52),
    CardBg = Color3.fromRGB(60, 60, 66),
    CardHover = Color3.fromRGB(75, 75, 82),
    CardBorder = Color3.fromRGB(120, 120, 130),
    Text = Color3.fromRGB(235, 235, 240),
    TextDim = Color3.fromRGB(160, 160, 170),
    TextDark = Color3.fromRGB(20, 20, 24),
    Status = Color3.fromRGB(160, 160, 170),
    Green = Color3.fromRGB(140, 200, 160),
    AccentGreen = Color3.fromRGB(140, 200, 160),
    AccentBlue = Color3.fromRGB(150, 170, 190),
    AccentPurple = Color3.fromRGB(160, 150, 180),
    AccentPink = Color3.fromRGB(190, 150, 165),
    AccentYellow = Color3.fromRGB(200, 190, 140),
    AccentRed = Color3.fromRGB(200, 120, 120),
    AccentOrange = Color3.fromRGB(200, 160, 120),
    ButtonESP = Color3.fromRGB(160, 160, 170),
    ButtonAim = Color3.fromRGB(180, 140, 140),
    FOVColor = Color3.fromRGB(180, 180, 190),
    Warning = Color3.fromRGB(200, 120, 120),
    Corpse = Color3.fromRGB(150, 145, 165),
    Pred = Color3.fromRGB(150, 170, 190),
    Team = Color3.fromRGB(150, 170, 190)
}
    ...
    if okTeams and teamsService then
        for _, team in ipairs(teamsService:GetTeams()) do
            if isValidTeamValue(team.Name) then
                local hasMe, hasThem = false, false
    ...
    for _, pair in ipairs(checkContainers) do
        ...
        if myC and theirC then
            for _, key in ipairs(teamValueKeys) do
                ...
                if myVal and theirVal and myVal:IsA("ValueBase") and theirVal:IsA("ValueBase") then
...
-- Shared env table (modules receive this)
local env = {
    Players = Players,
...
local function showBootError(msg)
    warn("[UtilityHub] ❌ " .. tostring(msg))
    print("[UtilityHub] ❌ " .. tostring(msg))
    logBoot("BOOT FAILED: " .. tostring(msg))
    if Cleanup then pcall(function() Cleanup.run() end) end
    ...
    pcall(function()
        ...
        gui.Parent = getSafeParent()
        local lbl = Instance.new("TextLabel")
        lbl.Size = UDim2.new(1, -60, 0, 280)
        lbl.Position = UDim2.new(0, 30, 0.25, 0)
...
-- ============================================================================
-- MODULE LOADER (compile each module source separately via loadstring/load)
-- ============================================================================

local loadstringFn = loadstring
if not loadstringFn and load then
    loadstringFn = function(src, chunkname)
        return load(src, chunkname)
    end
end
if not loadstringFn then
    showBootError("Executor ไม่มี loadstring / load — ใช้ไม่ได้")
    return
end

local factories = {}

local function registerModule(name, source)
    logBoot("compile " .. name)
    local chunk, compileErr = loadstringFn(source, "@UtilityHub/" .. name)
    if not chunk then
        error("compile โมดูล '" .. name .. "' ล้มเหลว: " .. tostring(compileErr))
    end
    local ok, factory = pcall(chunk)
    if not ok then
        error("execute โมดูล '" .. name .. "' ล้มเหลว: " .. tostring(factory))
    end
    if type(factory) ~= "function" then
        error("โมดูล '" .. name .. "' ต้อง return function(env) แต่ได้ type=" .. type(factory))
    end
    factories[name] = factory
    logBoot("compiled " .. name)
end


local compileOK, compileErr = pcall(function()
-- ========== MODULE SOURCE: cleanup ==========
registerModule("cleanup", [=[
return function(env)
...
registerModule("logger", [=[return function(env)
...
-- ========== MODULE SOURCE: ui_factory ==========
registerModule("ui_factory", [=[return function(env)
    ...
    UIF.Theme = {
        -- Frosted Glass Gray (translucent UX)
        Bg = Color3.fromRGB(40, 40, 44),
        Sidebar = Color3.fromRGB(48, 48, 52),
        CardBg = Color3.fromRGB(60, 60, 66),
        CardHover = Color3.fromRGB(75, 75, 82),
        CardBorder = Color3.fromRGB(130, 130, 140),
        AccentGreen = Color3.fromRGB(150, 200, 165),
        AccentBlue = Color3.fromRGB(155, 175, 195),
        AccentPurple = Color3.fromRGB(165, 155, 185),
        AccentPink = Color3.fromRGB(190, 155, 170),
        AccentYellow = Color3.fromRGB(200, 190, 145),
        AccentRed = Color3.fromRGB(200, 125, 125),
        AccentOrange = Color3.fromRGB(200, 165, 125),
        Text = Color3.fromRGB(235, 235, 240),
        TextDim = Color3.fromRGB(155, 155, 165),
        TextDark = Color3.fromRGB(22, 22, 26),
        SwitchOff = Color3.fromRGB(55, 55, 62),
        DangerBg = Color3.fromRGB(70, 45, 48),
        DangerHover = Color3.fromRGB(90, 55, 58),
        -- Transparency helpers (0 = opaque, 1 = invisible)
        PanelTransparency = 0.22,
        CardTransparency = 0.28,
    ...
    function UIF.button(parent, opts)
        ...
        btn.BackgroundTransparency = opts.Transparency or (UIF.Theme.CardTransparency or 0.28)
    ...
    function UIF.setButtonState(btn, active, activeText, inactiveText)
        ...
        if active then
            ...
            btn.BackgroundColor3 = UIF.Theme.CardBg
            btn.BackgroundTransparency = UIF.Theme.CardTransparency or 0.28
    ...
    function UIF.toggleCard(parent, opts)
        ...
        card.BackgroundTransparency = UIF.Theme.CardTransparency or 0.28
...
-- ========== MODULE SOURCE: hub ==========
registerModule("hub", [=[return function(env)
    local UserInputService = env.UserInputService
    local Cleanup = env.Cleanup
    local UIF = env.UIF
    local Colors = env.Colors
    ...
    -- กัน UI หายบน Delta iPad: ถ้าย้าย parent ช้า ให้ retry
    task.defer(function()
    ...
    -- ====== Main Window ======
    -- ขนาด UI ปรับตามจอ (iPad / มือถือ / PC)
    local vp = workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize or Vector2.new(800, 600)
    local isTouch = UserInputService.TouchEnabled
    local mainW, mainH = 530, 370
    if isTouch then
        -- iPad / มือถือ: ไม่ให้เกิน 92% ความกว้างจอ และสูงไม่เกิน 78%
        mainW = math.clamp(math.floor(vp.X * 0.88), 320, 560)
        mainH = math.clamp(math.floor(vp.Y * 0.72), 280, 420)
    end

    local Main = Instance.new("Frame")
    Main.Size = UDim2.new(0, mainW, 0, mainH)
    Main.Position = UDim2.new(0.5, -math.floor(mainW/2), 0.5, -math.floor(mainH/2))
    Main.BackgroundColor3 = UIF.Theme.Bg
    Main.BackgroundTransparency = UIF.Theme.PanelTransparency or 0.22
    ...
    OpenBtn.TextColor3 = UIF.Theme.Text
    OpenBtn.TextSize = 13
    ...
    openStroke.Color = UIF.Theme.CardBorder
    ...
    Header.BackgroundTransparency = (UIF.Theme.SidebarTransparency or 0.18) * 0.5
    ...
    headerBorder.BackgroundTransparency = 0.45
    ...
    VersionBadge.BackgroundColor3 = UIF.Theme.CardBorder
    VersionBadge.BackgroundTransparency = 0.55
    VersionBadge.Text = "v2.0"
    VersionBadge.TextColor3 = UIF.Theme.Text
    ...
    Sidebar.BackgroundTransparency = UIF.Theme.SidebarTransparency or 0.18
    ...
    local function createTab(key, name, icon)
        ...
        btn.Activated:Connect(function()
            for k, p in pairs(tabs) do
                ...
                tabButtons[k].btn.BackgroundTransparency = (k == key) and 0.25 or 0.45
    ...
    tabButtons["combat"].btn.BackgroundTransparency = 0.25
    ...
    NotifFrame.BackgroundTransparency = 0.18
    NotifFrame.BorderSizePixel = 0
    NotifFrame.Visible = false
    NotifFrame.Active = true
    Instance.new("UICorner", NotifFrame).CornerRadius = UDim.new(0, 8)
    local notifStroke = Instance.new("UIStroke", NotifFrame)
    notifStroke.Color = UIF.Theme.CardBorder
    ...
    MobileHub.Size = UDim2.new(0, 160, 0, 260)
    MobileHub.Position = UDim2.new(1, -170, 0.5, -130)
    MobileHub.BackgroundColor3 = UIF.Theme.Bg
    MobileHub.BackgroundTransparency = 0.2
    ...
    mhStroke.Color = UIF.Theme.CardBorder
    mhStroke.Thickness = 1.2
    mhStroke.Transparency = 0.3
    ...
    MobileToggle.BackgroundColor3 = UIF.Theme.CardBg
    MobileToggle.BackgroundTransparency = 0.15
    MobileToggle.Text = "🕹️"
    MobileToggle.TextSize = 20
    MobileToggle.TextColor3 = UIF.Theme.Text
    ...
    local function createMobileBtn(text, color, callback)
        ...
        Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 8)
        -- ใช้ InputBegan เสริมบนมือถือ (บางเวอร์ชัน Delta Activated ไม่ยิง)
        btn.Activated:Connect(callback)
        btn.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
                -- ไม่เรียกซ้ำถ้า Activated ทำงานแล้ว — ใช้ debounce เล็กน้อย
            end
        end)
    end
...
-- ========== MODULE SOURCE: esp ==========
registerModule("esp", [=[return function(env)
    local Players = env.Players
    local RunService = env.RunService
    local LocalPlayer = env.LocalPlayer
    local Cleanup = env.Cleanup
...
-- ========== MODULE SOURCE: aimbot ==========
registerModule("aimbot", [=[return function(env)
    local Players = env.Players
    local RunService = env.RunService
    local UserInputService = env.UserInputService
...
-- ========== MODULE SOURCE: world ==========
registerModule("world", [=[return function(env)
    local RunService = env.RunService
    local UserInputService = env.UserInputService
    local LocalPlayer = env.LocalPlayer
    local Cleanup = env.Cleanup
...
-- ========== MODULE SOURCE: godmode ==========
registerModule("godmode", [=[-- ============================================================================
...
registerModule("config", [=[return function(env)
...
-- ========== MODULE SOURCE: antitamper ==========
registerModule("antitamper", [=[-- ============================================================================
...
-- ========== MODULE SOURCE: dumper ==========
registerModule("dumper", [=[return function(env)
    local Dumper = {}
...
if not compileOK then
    showBootError("Compile modules failed: " .. tostring(compileErr))
    return
end
...
local bootOK, bootErr = pcall(function()
    local function loadModule(name)
        ...
        if type(factory) ~= "function" then
            error("โมดูล '" .. name .. "' ไม่พบ factory (type=" .. type(factory) .. ")")
        end
    ...
    setBootMarker("🛠️ โหลด: cleanup...")
    Cleanup = loadModule("cleanup")
    env.Cleanup = Cleanup

    setBootMarker("🛠️ โหลด: logger...")
    ...
    _G.UtilityHubCleanup = function()
        pcall(function() Cleanup.run() end)
        for k in pairs(FriendlyPlayers) do
            FriendlyPlayers[k] = nil
        end
    end
    ...
    -- Exit button
    if UI.buttons and UI.buttons.exit then
    ...
    task.delay(1.8, destroyBootMarker)

    if UI.showNotif then
        UI.showNotif("✅ UtilityHub v2.0 พร้อมใช้งาน (Single File)", 3)
    end
end)
